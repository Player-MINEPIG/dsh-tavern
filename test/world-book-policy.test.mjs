...
test('matches literal keys and requires explicit unsafe opt-in for JavaScript regex keys', () => {
  ...
  assert.equal(matchWorldBookKey('moon', '/moon/g', { allowUnsafeRegex: true }).code, 'unsupported-regex-flags')
  assert.equal(matchWorldBookKey('moon', '/moon/ii', { allowUnsafeRegex: true }).code, 'unsupported-regex-flags')
  assert.equal(matchWorldBookKey('moon', '/moon/z', { allowUnsafeRegex: true }).code, 'unsupported-regex-flags')
