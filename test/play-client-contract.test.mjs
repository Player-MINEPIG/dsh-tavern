...
test('live play client parses JSON file envelopes and writes them as content strings', async () => {
  ...
  const selectionPost = calls.find(item => item.method === 'POST' && item.url === API_V1 + '/character-selection')
  assert.equal(JSON.parse(selectionPost.body).character.greetingIndex, 1)
