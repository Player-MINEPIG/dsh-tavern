...
test('live play client parses JSON file envelopes and writes them as content strings', async () => {
  ...
  await client.relinkPlaythroughCharacter('playthrough-1', 'character-2')
  const relink = calls.find(item => item.url.endsWith('/playthroughs/playthrough-1/relink-character'))
  assert.equal(relink.method, 'POST')
  assert.deepEqual(JSON.parse(relink.body), { characterId: 'character-2' })
