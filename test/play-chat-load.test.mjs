...
test('swipe controls always expose position and project an immediate thinking state while generating', () => {
  ...
  assert.match(actionsSource, /dtv-play-turn-position'[\s\S]*displayedPosition \+ 1[\s\S]*displayedVariantCount/)
  assert.match(actionsSource, /displayedPosition = pendingVariant \? turn\.variants\.length : position/)
  assert.match(actionsSource, /displayedVariantCount = pendingVariant \? turn\.variants\.length \+ 1 : turn\.variants\.length/)
  assert.match(chatSource, /pendingVariant: swipePending/)
