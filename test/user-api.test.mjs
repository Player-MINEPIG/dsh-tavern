...
test('user API performs CRUD, isolates session bindings, unbinds, and clears deleted ids', async () => {
  ...
  try {
    const first = await invoke(handler, { method: 'POST', url: '/pmp-dsh-tavern/api/v1/users', body: { id: 'reader-a', name: 'Reader A', description: 'First description.' } })
    const second = await invoke(handler, { method: 'POST', url: '/pmp-dsh-tavern/api/v1/users', body: { id: 'reader-b', name: 'Reader B', description: 'Second description.' } })
    assert.equal(first.status, 201)
    assert.equal(second.status, 201)
    assert.deepEqual(Object.keys(first.json.user), ['id', 'name', 'description'])

    const updated = await invoke(handler, { method: 'PATCH', url: '/pmp-dsh-tavern/api/v1/users/reader-a', body: { name: 'Reader A+', description: 'Immediate refresh text.' } })
    assert.equal(updated.json.user.description, 'Immediate refresh text.')
    assert.equal((await invoke(handler, { url: '/pmp-dsh-tavern/api/v1/users' })).json.users.length, 2)

    await invoke(handler, { method: 'POST', url: '/pmp-dsh-tavern/api/v1/user-selection', body: { sessionId: 'session-a', userId: 'reader-a' } })
    await invoke(handler, { method: 'POST', url: '/pmp-dsh-tavern/api/v1/user-selection', body: { sessionId: 'session-b', userId: 'reader-b' } })
    assert.equal((await invoke(handler, { url: '/pmp-dsh-tavern/api/v1/user-selection?sessionId=session-a' })).json.user.name, 'Reader A+')
    assert.equal((await invoke(handler, { url: '/pmp-dsh-tavern/api/v1/user-selection?sessionId=session-b' })).json.user.name, 'Reader B')

    await invoke(handler, { method: 'POST', url: '/pmp-dsh-tavern/api/v1/user-selection', body: { sessionId: 'session-a', userId: null } })
    assert.equal((await invoke(handler, { url: '/pmp-dsh-tavern/api/v1/user-selection?sessionId=session-a' })).json.selection, null)

    await invoke(handler, { method: 'DELETE', url: '/pmp-dsh-tavern/api/v1/users/reader-b', body: {} })
    assert.equal(new SessionSelectionStore(directory).get('session-b').userId, null)
    assert.equal((await invoke(handler, { url: '/pmp-dsh-tavern/api/v1/user-selection?sessionId=session-b' })).json.selection, null)
...
test('root Tavern API dispatcher exposes user routes through the single secured prefix', async () => {
  ...
  try {
    ...
    const result = await invoke(route.handler, {
      url: '/pmp-dsh-tavern/api/v1/users',
      headers: { host: 'localhost:53100' },
    })
    assert.equal(route.path, '/pmp-dsh-tavern/api')
    assert.equal(result.status, 200)
    assert.equal(result.json.users[0].id, 'root-user')
    const relation = await invoke(route.handler, {
      method: 'PUT',
      url: '/pmp-dsh-tavern/api/v1/users/root-user/world-books',
...
test('user API rejects unsupported fields and blocks a running-session switch', async () => {
  ...
  try {
    const avatar = await invoke(handler, { method: 'POST', url: '/pmp-dsh-tavern/api/v1/users', body: { name: 'Bad', description: '', avatar: 'forbidden' } })
    assert.equal(avatar.status, 400)
    assert.equal(avatar.json.error.code, 'INVALID_USER_REQUEST')
    const blocked = await invoke(handler, { method: 'POST', url: '/pmp-dsh-tavern/api/v1/user-selection', body: { sessionId: 'running', userId: 'safe-user' } })
...
test('user API edits bounded world-book relationships without changing the three-field user document', async () => {
  ...
  try {
    const saved = await invoke(handler, {
      method: 'PUT',
      url: '/pmp-dsh-tavern/api/v1/users/reader-a/world-books',
    ...
    assert.deepEqual((await invoke(handler, { url: '/pmp-dsh-tavern/api/v1/users/reader-a/world-books' })).json.binding.worldBookIds, ['book-a', 'book-b'])
    ...
    await invoke(handler, { method: 'DELETE', url: '/pmp-dsh-tavern/api/v1/users/reader-a', body: {} })
