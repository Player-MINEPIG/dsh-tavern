...
test('play use-case owns chrome/workspace/timeline logic and leaves Host seams to the loader', () => {
  ...
  const playHost = read('../packages/tavern-loader/src/play-host.js')
  assert.doesNotMatch(playHost, /apiProxy/)
  assert.match(playHost, /sessionController/)
  assert.match(playHost, /workspaceController/)
  assert.match(playHost, /directoryPickerController/)
  assert.doesNotMatch(playHost, /archiveSession/)
})
