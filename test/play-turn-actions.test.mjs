...
test('right swipe adopts the next variant or generates one at the end', () => {
  ...
  assert.match(source, /queueSwipeTransition\(result\.sessionId, targetPosition < position \? 'previous' : 'next'\)/)
  assert.match(source, /queueSwipeTransition\(result\.sessionId, 'next'\)/)
