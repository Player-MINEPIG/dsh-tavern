...
function sectionSnapshot(section, variables, known, index) {
  ...
  return { name: section.name, index, text, ...counts(text), hash: digest(text),
    provenance: origin?.provenance ?? 'unknown', sources: origin?.sources ?? [] }
}
const messageText = message => (message.content ?? []).filter(b => b.type === 'text').map(b => b.text).join('')
...
export class AssemblyRecorder {
  ...
  begin({ agent, turn, step, snapshot, legacyRecord }) {
    ...
    const record = { schemaVersion: 3, id: randomUUID(), sessionId: id, turn, step,
      attempt: Math.max(0, ...this.store.list(id).filter(r => r.turn === turn && r.step === step).map(r => r.attempt)) + 1,
      recordedAt: Date.now(), status: 'assembled', contentStatus: 'available',
    ...
    try {
      ...
      let offset = 0
      for (const section of record.sections) { section.offsetUtf16 = offset; offset += section.text.length + 2 }
  ...
  request(options, session) {
    ...
    const expected = record.sections.map(s => s.text).join('\n\n')
    const indices = systems.flatMap((text, index) => text === expected ? [index] : [])
    // Only an exact unique whole-message match establishes output offsets.
    const verified = record.contentStatus === 'available' && indices.length === 1
  ...
  failure({ agent, turn, step }) {
    ...
    this.store.put({ schemaVersion: 3, id: randomUUID(), sessionId: id, turn, step, attempt: 1,
