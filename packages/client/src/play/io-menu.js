...
import { PLUGIN_ID, CLIENT_REFRESH_EVENT } from '../../../identity.js'
import {
...
export function PlayIoMenu({ playClient, playthrough, trigger = '+', placement = 'composer', onRelink }) {
  ...
  /**
   * Removing a playthrough is the one destructive action in this menu, so it
   * always asks first. The host keeps the files in `.dtavern-trash/` and never
   * touches the bound DSH sessions; a host without the opt-in answers 403 and a
   * playthrough with a live turn answers 409, and both are shown as guidance
   * rather than as a raw error code.
   */
  const remove = async () => {
    if (busy) return
  ...
  return h('div', { ref: root, className: 'dtv-play-io', 'data-placement': placement },
    ...
    !open ? null : h('div', { className: 'dtv-play-io-menu' },
      ...
      h('button', { type: 'button', className: 'dtv-play-io-item dtv-play-io-danger', disabled: busy, onClick: remove }, uiMessage('play.io.delete')),
      error === '' ? null : h('p', { className: 'dtv-play-io-error', role: 'alert' }, rawText(error)),
      done === '' ? null : h('p', { className: 'dtv-play-io-done', role: 'status' }, rawText(done)),
    ),
  )
}
