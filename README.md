...
> 当前源码准备发布 `2.3.2`，修复回退、分支和非首轮 swipe 继承待处理输入的问题；目标运行环境仍为 DSH `0.1.5-rc.1`，新版 DSH 兼容性尚未验证。见 [变更记录](CHANGELOG.md)。项目代码采用 [MIT License](LICENSE)。
>
...
当前已发布版本为 `2.3.1`，尚不包含上述修复；`2.3.2` 仍在发布准备中。安装已发布版本请使用固定标签：
