...
下一阶段才接管对话表面：蓝球 **灵珠**（`lingzhu`）= DSH 原前端，红球 **魔丸**（`mowan`）= ST 风格前端；两边共用的工作区 / 局用 `lzmw`。Swipe 会走 DSH 分支并新开 session，以便旧回复不再进入后续上下文。这会让会话日志明显变大：第一次选择灵珠魔丸工作区时会提示不要放在系统盘或空间吃紧的位置。局的导入/导出、按需复制的局分支也属于这一阶段，尚未随 `1.0.0` 提供。魔丸时间线由插件按固定路径读 DSH 会话与指针，不经过模型工具，因此 v1 RP 锁不会拦显示。人类可读导出走静态 HTML（导出时套用当前正则，文件内不内嵌脚本）；另带 SillyTavern 聊天 JSON。
