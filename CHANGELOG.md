...
- Clarify unstarted playthrough preparation and valid greeting counts; lock greeting selection and import together once conversation or import-claim evidence exists.
...
- Keep bilingual product/technical documentation and the editable architecture diagram aligned with the current implementation. Acceptance pages present one current result and remaining checks; older documentation is available at its Git tag.

- Fix greeting navigation treating empty alternate entries as selectable openings. Skip blank alternatives without renumbering card indices, disable each direction at its boundary, and retain a recovery control for an already selected blank opening. Apply the same behavior to RP chat and the opening dock.
- Accept character-card `tags: null` as empty normalized metadata with a diagnostic, preserving source JSON and exports. Cover V1/V2/V3, dual PNG chunks, editing and malformed tag rejection. Verified an affected PNG through import, persistence/reload and lossless JSON export.
...
- On 2026-09-11, the user accepted the rendering fixes in the temporary DSH `0.1.5-rc.1` environment. All 191 installed package files matched the candidate; configuration and sessions were retained. Automated checks: 541 pass, 5 optional/external integration skips; 22 Chrome checks pass. Template JavaScript remains blocked. See [acceptance records at v2.2.0](https://github.com/Player-MINEPIG/dsh-tavern/blob/v2.2.0/docs/PLAY_REVIEW_en.md#220-rich-text-rendering-acceptance-2026-09-11).
