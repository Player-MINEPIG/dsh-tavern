...
| DELETE | `/playthroughs/:id` | **Optional verb, off by default**: move the playthrough directory to a `.dtavern-trash/<timestamp>-<id>/` backup beside it, drop its catalog row, and append a body-free tombstone; responds `{ removed, backupDir, sessionIds, rootSessionId }`. A failed backup never leaves a half-deleted playthrough. **No DSH session is deleted, archived, or renamed**; only the Tavern selection of that playthrough's sessions is cleared. A host without `allowPlaythroughDelete: true` → 403 `PLAYTHROUGH_DELETE_DISABLED`; unknown `:id` → 404 `PLAYTHROUGH_NOT_FOUND`; a bound session still generating → 409 `PLAYTHROUGH_AGENT_RUNNING`, which leaves the directory, catalog, and backups untouched | Added (off by default) |
...
`DELETE /playthroughs/:id` is the only verb that removes a playthrough, so it is off by default and it finishes **only at the playthrough layer**: the backup directory sits beside the playthrough (`.dtavern-trash/`, movable back by hand to restore it), and the catalog only loses one row while `ext.pmpDshTavern.deletedPlaythroughs` keeps a body-free tombstone as a breadcrumb. The returned `sessionIds` / `rootSessionId` exist for a **session-owning** surface (an archive/memory plugin, for instance): deleting, archiving, or renaming a DSH session is not this path's job and should not be started by this plugin's loader. The split follows the `play` layer boundary in `docs/ARCHITECTURE.md` and the same sentence the detach contract already states.
