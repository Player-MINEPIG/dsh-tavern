...
在当前目标 DSH `0.1.7-alpha.2` 中，RP 会对已公开的会话错误或最新回合的最终失败显示：“出现错误，请切换到「对话」视图查看更多信息。”点击 DSH 顶部的“对话”页签查看具体原因；RP 不自动切换视图，也不重复展示提供方诊断。提示随 Tavern UI 语言变化。新请求运行期间不显示上一回合的失败，后续成功或主动取消不会持续显示旧错误；自动重试中和可恢复的工具错误本身不等同于最终失败。
...
- 预设、角色卡、用户、世界书、会话模板、显示正则或外部记录完全无法导入时，界面会在原有行内错误之外显示“导入失败”弹窗；若资源已成功导入、只是附带兼容性诊断或警告，则只保留面板中的诊断，不弹失败窗。
...
预设面板支持导入 SillyTavern Chat Completion preset JSON，也可以创建空白预设。编辑保存后可通过“导出 JSON”下载当前资源。

推理强度可选择继承、关闭（`off`）、低/中/高/额外高和最高（`max`）。导入文件中的其他合法 provider effort 值也会保留，并在下拉框中显示；修改其他字段不会清除它。选择继承会清除 Tavern 的显式覆盖。实际可用值仍由目标模型/provider 决定；资源中保留原值不代表每个模型都支持该值。
...
4. 用户可绑定零本或多本独立世界书。用户正文和世界书关系是两个独立保存动作，面板会提示未保存修改。
5. 保存后再绑定/更新到当前 session。解绑用户会移除用户描述及其世界书来源，但不会删除 session 自己显式选择的世界书。
...
- “新建空白模板”不依赖当前会话。选择模板后，可直接编辑名称、预设、角色卡、用户、有序世界书、开场序号、角色提示词偏好和已保存的 RP 设置，再点击“保存修改”；这不会改变当前会话绑定；
- “由当前设置创建”与“用当前设置更新”仍可获取当前 session 的实际设置。使用这两个入口前，请先在资源面板中保存并应用配置；
- “导出 JSON”保存已保存的模板名称、设置和资源 ID 引用，不打包被引用资源。导入会创建新模板并保留原来的默认模板选择；从列表中明确选择导入的模板，再检查引用与诊断。不同资料库之间不会按名称自动重映射资源；
...
真实用户触发的回复使用 ST 式左右 swipe：序号从初始回复起始终显示（初始为 `1/1`）；左箭头只采用已有上一项；右侧存在已有项时采用下一项，已经位于最后一项时同一个右箭头自动变为“再试一次”，创建并采用一个新 swipe，不另外显示星形生成按钮。点击“再试一次”后，新 DSH session 创建成功即自动打开，无需等整条回复完成；等待期间再次点击周目或切换到原生“对话”视图，也使用该新 session。RP 视图显示此前的上下文、此次用户消息与新回复的流式正文，可用原生输入栏的“停止生成”中断。完成前不会把临时文本写成 timeline 变体；结束后仅按 DSH 已保存的用户/助手消息坐标采用新变体，中断后已有的持久回复也按此处理。失败且没有可保存回复时显示错误，可点击“返回已有回复”，旧变体保持不变。

等待期的导航关联属于当前前端实例；刷新或关闭页面会丢失该临时关联，DSH 新会话仍保留，可从原生会话入口查看和中断。
...
生成期间，当前周目所有已完成回复的切换变体、生成 swipe、回退和分支新周目按钮暂时禁用，鼠标悬停时提示“模型正在输出中，暂不可操作”。复制、修改显示文字、恢复原回复、切换会话或视图和停止生成仍可用；其他周目的操作不等待本周目生成完成。显示文字保存只修改对应元数据，不改变正在生成的上下文，生成完成、失败或中断后恢复路径操作。
