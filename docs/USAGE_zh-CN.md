...
在当前目标 DSH `0.1.7-rc.1` 中，RP 会对已公开的会话错误或最新回合的最终失败显示：“出现错误，请切换到「对话」视图查看更多信息。”点击 DSH 顶部的“对话”页签查看具体原因；RP 不自动切换视图，也不重复展示提供方诊断。提示随 Tavern UI 语言变化。新请求运行期间不显示上一回合的失败，后续成功或主动取消不会持续显示旧错误；自动重试中和可恢复的工具错误本身不等同于最终失败。
