...
DSH `0.1.0-rc.8` 侧边栏外层的“新建会话”属于原生 sidebar shell，当前公开扩展合同不能拦截或替换其点击。为避免依赖哈希类名或全局 DOM 监听，魔丸保留该按钮的原生行为，且不推荐在魔丸模式中使用。魔丸侧边栏“普通 / 非角色扮演会话”右侧的 `+` 只弹出说明，可关闭或切回 DSH 原生模式；它不会暗中创建、移动或改名 session。周目应从角色卡右侧的 `+` 创建。
