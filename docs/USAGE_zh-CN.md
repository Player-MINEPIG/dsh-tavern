...
生成期间，当前周目所有已完成回复的切换变体、生成 swipe、回退和分支新周目按钮暂时禁用，提示“此周目正在生成，请先停止”。复制、修改显示文字、恢复原回复、切换会话或视图和停止生成仍可用；其他周目的操作不等待本周目生成完成。显示文字保存只修改对应元数据，不改变正在生成的上下文，生成完成、失败或中断后恢复路径操作。
