...
状态：2026-08-14 纯模块已完成，随后已集成到首个公开发布候选版本；模块证据以 `ACCEPTANCE.md` 为准，整体证据见 `../INTEGRATION_ACCEPTANCE.md`。

## 文件布局

- `packages/world-book/src/format.js`：识别、JSON 解析、诊断、标准化模型、双格式导出、稳定序列化。
- `packages/world-book/src/policy.js`：普通 key 匹配、默认阻断并诊断原生正则 key、secondary logic、稳定排序、显式概率 roll/group roll/预算输入下的候选计算。只有调用方明确传入 `allowUnsafeRegex: true` 时才执行 JavaScript `RegExp`。
...
- 正则：默认以 `unsafe-regex-disabled` 返回而不执行；不安全兼容模式下，超长或无效正则也只让该 key 不匹配，并在 entry evaluation 的 `invalidKeys` 返回原因，不抛出使整本书失效。
