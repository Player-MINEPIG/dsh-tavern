...
已验收的实现边界是：greeting、导入 QA 和 timeline 均不伪造 DSH durable message；首次实际请求才通过 loader 注入转义且标明 `untrusted` 的只读上下文，正常 `turn/end` 后标记 consumed。这个流程不构成跨标签页的 catalog/timeline CAS，也不构成跨文件周目创建事务；并发写入、失败补偿、超过 32 页 history 的完整性和 request abort 后的一次性 claim 仍见 `PLAY_REVIEW.md`，不得在第三方协议中宣称已经解决。
