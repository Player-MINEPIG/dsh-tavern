...
兼容性 bug 修复可使用补丁版本；单纯纠正文档不必升版。一次修复 PR 可以先合并，版本号在确定发布范围时统一更新。正式对外发布时，包版本、Git tag、GitHub Release 和固定版本安装示例应一致，已发布的 tag 不得移动或覆盖。

| 文件或发布内容 | 何时更新 |
| --- | --- |
| 根目录 `package.json`、`package-lock.json` | 发布准备时同步版本；只改文档无需升版 |
| 根目录 [CHANGELOG.md](../CHANGELOG.md) | 记录修复、用户影响及兼容边界；发布前标为未发布，正式发布时改为发布版本和日期 |
| 中英文 README、INSTALLATION | 同步当前源码/发布状态；正式发布时将安装及源码检出示例切换到新 tag，注明目标 DSH 与迁移要求 |
