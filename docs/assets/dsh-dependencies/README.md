...
| DSH CLI/profile 布局、`dshHomePath()`、官方格式迁移包 | 安装/迁移运维路径：高；日常资源 CRUD 不直接依赖 codec | 安装脚本依赖 profile 及包目录布局；坐标迁移显式检查格式库 `0.1.7-alpha.1` 或 `0.1.7-rc.1`，使用官方 V0→V1→V2→V3→V4 codec/catalog，不是通用未来迁移器。发布要求显式子日志证据、精确后继验证、全部文件先备份与幂等格式标记；V3 之前的 header-body Trace 转换拒绝，不提供回滚工具。[install](../../../scripts/install.mjs)、[shared](../../../scripts/shared.mjs)、[coordinate migration](../../../scripts/migrate-session-coordinates.mjs)、[迁移说明](../../DSH_0.1.7_MIGRATION.md) |
