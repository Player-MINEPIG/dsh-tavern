...
4. Release gate: `npm run verify:2.0` already includes build and pack dry-run. Recheck dependency audit, public-path/secret scan, official docs, and version. See the `2.1.0` acceptance status above; merge and create the `v2.1.0` tag only after final review and authorization.
