...
`2.1.0-rc.2` migrates automated compatibility regression to the DSH `0.1.2-rc.1` controllers, stable history cut, Session snapshots, and client contract owners. The `0.1.0-rc.8` paragraph above remains historical human-acceptance evidence; it does not claim that the new version's manual pass is done. Recheck the real provider, streaming/steer/cancel/retry/fork, launcher behavior after first credential entry, and both UI locales below.
...
4. Release gate: `npm run verify:2.0` already includes build and pack dry-run. Recheck dependency audit, public-path/secret scan, official docs, and version; promote `2.1.0-rc.2` to the final `2.1.0` tag only after candidate acceptance passes.
