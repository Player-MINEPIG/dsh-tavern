...
While generating, variant switching, new swipes, rollback, and new-playthrough forks are disabled on all completed replies in the current playthrough, with the hover tooltip “The model is generating output. This action is temporarily unavailable.”. Copy, display editing, restoring original text, session/view navigation, and stopping generation remain available. Other playthroughs do not wait for this generation. Display saves only change metadata, leaving the in-flight model context intact; path actions become available after completion, failure, or interruption.
