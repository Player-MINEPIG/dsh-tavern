...
Opening preparation follows conversation history, open turns and import claim state, not the native empty-session screen. A playthrough with only permission/title initialization events still shows that it has not started and allows record import. The UI states the number of non-empty greetings; blank alternatives do not count and a single greeting needs no switching. Existing conversation or an import claim locks both greeting selection and import actions.
