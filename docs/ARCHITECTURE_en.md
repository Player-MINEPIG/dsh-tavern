...
The current contract targets this repository's source (version in [package.json](../package.json)) and DSH `0.1.7-rc.1`.
...
The presentation layer reads the latest turn's `turn/end.reason.kind === 'error'` through the public DSH `0.1.7-rc.1` `useChat` timeline and combines it with `useSession`'s `promptError`, `lastAgentError`, and `openError`. One Tavern-localized notice directs users to native Chat for detailed diagnostics; no provider-message matching or duplicate error history is introduced. New submissions and active generation hide the previous terminal notice; a later open, successful or cancelled turn supersedes old failures. Current Session errors remain visible even while busy. Browser crashes and connection failures that never reach these public outlets are outside this notice's guarantees.

## Current workspace diagnostics

The `packages/client` composition root owns one diagnostics controller. The sidebar and **DT → Diagnostics** share workspace, catalog and timeline reads; switching modes does not recreate diagnostics state. The panel works in native and play modes, with causes, recovery suggestions, expandable technical details, recheck and JSON report copying. The sidebar shows only a dismissible summary and warning buttons on affected playthroughs.

Diagnostics cover both failed file reads and readable timelines without an available Session. The latter reuse the sidebar's workspace-membership and archive projection after both official `useSessions` / `useWorkspaces` mirrors reach `ready`. An empty timeline still requires a usable root Session; neither row order nor a successful read proves availability. A mirror that is still loading does not prove absence. Official state changes recompute issues, resolved issues disappear, and a newer read invalidates older asynchronous results.

This is a current-problem projection, not a history log or a new HTTP API. Bounded `sessionStorage` entries hold only dismissed-summary identities (workspace, playthrough and error code), not error bodies, resources or conversations. Dismissal keeps issues and warning buttons available across refreshes and mode switches; a confirmed resolution followed by recurrence alerts again. Copied reports contain workspace paths, playthrough/Session identifiers and error details, which should be reviewed before sharing.
...
| `tavern-loader` | “How do current resources affect this DSH request?” | Assemble the selected preset, map supported call config, append/replace policy, Host/API mount, exclusive pending-input projection, RP session overlay | Reinterpreting raw ST fields, implementing concrete UI |
...
The unified loader expands its profile registration into named `pmp-dsh-tavern:part:*` sections, keeps import context and optional `rp:policy` separate, and introduces loader-owned `SessionSelectionStore`. Preset, character, user, and world-book documents stay in their modules, but “which session uses which resources” and the RP overlay are persisted by one policy. Ordinary forks and delegated subagents both copy the parent selection. RP is not a DSH agent preset.
...
Clean sessions and configuration templates are stored by `packages/session-template` as a pure selection projection. The loader injects the real resource libraries and `SessionSelectionStore`. In DSH mode, the browser composition root creates/navigates an ordinary blank session only through public `uiWorkspace.connectWorkspace()` and `uiWorkspace.openSession()`. In Mowan, the same control plane first previews the configuration and obtains the character id, then reuses the shared playthrough controller and existing v2 atomic operations to create or reuse that character's playthrough, and finally writes the complete selection atomically through v1 apply. There is no dedicated “configure playthrough” backend verb. Neither path forks or forges history. Full transaction boundary: `LOADER_CONTRACT_en.md`.

## Loader-owned ActivationContext

DSH `0.1.7-rc.1` `agent/inbox/spliced` is a public, durable Session event. Insert, replace, cancel, and claim first append that event and synchronously notify `session/event`; only then does the live Inbox change. At this boundary the loader maintains the only `PendingInputProjection`; it uses `Session.ownEvents()` to exclude a fork's inherited prefix, then combines durable history with this step's claimed batch (after de-duplication) into a temporary `ActivationContext` for the world-book matcher to consume read-only. Other current-log reads are centralized on `session.seq` and `snapshotEvents()` and no longer access the removed `Session.events`.
...
- Failure reasons use an independent `failureRef` to an official failure event. Error bodies are not copied and the prompt-reference cut is not extended. Details return a reason only after reference verification; an unavailable failure reference does not invalidate prompt sections that can still be verified.
...
This projection lets current input participate in activation during the first system assembly. It does not change ownership of DSH durable history, real role-message order, tool permissions, `agent/request`, or `request/header`.
...
| Clean new session / templates | DSH mode reuses `uiWorkspace.connectWorkspace()`; Mowan reuses the playthrough v2 session-controller composition; both navigate with `uiWorkspace.openSession()` | Tavern only copies selection atomically onto the target session. Mowan additionally treats the configured character as playthrough ownership and read-back-validates. It does not construct messages or fork history |
...
| Prompt and audit | `systemPrompt.section`, `system-prompt/assemble`, `agent/request`, `llm/stream`, official Session events / inspect | The loader emits named sections. Trace stores bounded source metadata and official-history references, then cold-reads and verifies bodies on demand |
...
### Current Tavern frontend boundaries

The following behavior defines the current implementation and its public dependency boundary:
...
- A dedicated details block parser preserves inner Markdown without adding Markdown breaks to raw HTML layouts. Complete HTML documents in closed unlabeled/html fences render as static templates; ordinary code fragments stay literal. `rich-text-styles.js` places purified styled content in Shadow DOM, isolates complete documents individually and adapts their root selectors, with outer layout/paint containment. A React ref mounts it after insertion/updates; static exports use the same declarative roots. Template scripts remain blocked and no variable runtime is implemented. Browser verification: `node scripts/verify-rich-text-browser.mjs` (Chrome/Chromium; optionally set `CHROME_PATH`).
- Mowan does not render reasoning or runtime context and provides no expand control. Users who need runtime detail return to native DSH **Chat**. Action buttons may depend only on public icons, `Tooltip`, and similar interfaces; bundle-internal unpublished `ReasoningRow` and `MessageIconActions` are not dependable interfaces.
...
- Playthrough IO exists only in the sidebar `PlayIoMenu`; it does not occupy or rewrite native composer-left actions.
- Message scrolling uses the DSH scrollport's `scrollTop = scrollHeight` semantics. Tavern does not maintain local anchors, fixed occlusion values, or composer-height compensation.
...
DSH currently has no native data model for character cards, playthroughs, greeting, cross-session adopted variants, ST display regex, or Tavern selection. Therefore `catalog.json` / `timeline.json`, the character→playthrough sidebar projection, cross-session reply switching, opening display and first-turn prompt reference, regex resource management, and the v2 playthrough protocol stay owned by Tavern. They point at DSH-authoritative messages by event range and do not copy bodies. Subdirectories inside the RP workspace must also pass Tavern's path jail. DSH native/browse directory flow is a UI seam for choosing or registering a Workspace. It does not provide a unified Host capability for “let a third-party v2 frontend safely create arbitrary playthrough subdirectories inside an already-bound root”.

Playthrough names and DSH session names are intentionally separate. Tavern stores a monotonic number per character and displays `Playthrough {number}` or `{number}周目` for the current UI locale. After an explicit user rename, the title is shown verbatim and no longer changes with locale. The original DSH session stays named by Host as “character name + time”, so authoritative data stays recognizable after leaving Mowan or uninstalling. Old catalogs without a number get a computed fill from that card's existing order. Legacy generated titles with a number can be recognized at display time, but old rows are not rewritten only for compatibility.

Create only inspects that card's highest-numbered playthrough. Reuse is forbidden when the timeline already has QA, imported context already has QA, DSH-authoritative messages already have user/assistant, or an unfinished turn exists. Greeting alone is not durable history. Only when all four are empty is the original root session opened, with no directory, session, or catalog write. When an old playthrough references a missing Session, preserve that playthrough and its error while allowing a new playthrough. Never reuse or overwrite the missing run as empty. Other read errors propagate instead of being guessed as empty.
...
Imported records remain immutable `import-context.json`. They neither forge DSH user/assistant QA nor copy into the timeline; DSH official history does retain imported system text used in real requests. An empty session manages the loader's authoritative binding through `/sessions/:id/import-context` GET/PUT/DELETE. Rebind writes a new file then replaces the pending reference. Unbind only clears the reference and keeps the workspace file for recovery/audit. After DSH conversation, an open turn, or a claimed binding, the backend locks mutation. Public export composes “imported greeting/QA → later timeline pointers resolved to DSH originals”. Static HTML outputs the currently visible greeting and RP render. SillyTavern JSONL outputs greeting, the current active path, and each QA's `swipes` / `swipe_id`. Greeting `{{user}}` / `{{char}}` are frozen to current display names at export. Import files stay immutable and keep participating in hash checks. The loader expands Tavern macros in greeting and QA with the current user/character profile snapshot only at prompt-projection time, then XML-escapes so leftover ST placeholders are not misread as DSH prompt variables. ST JSONL cannot express the full playthrough tree. When this plugin re-imports an ST record it binds only the selected linear history represented by each line's `mes`. The current public surface defines no portable bundle. Full recovery requires the RP workspace, Tavern persistent directory, and corresponding DSH Session logs with their inheritance dependencies. The frontend must also sync catalog/timeline display references and read-back-validate. Current v2 has no unified transaction across import binding, files, and catalog/timeline, so validation can only expose a half-complete state. It cannot claim an atomic commit.
The current playthrough lifecycle is a frontend composition of these atomic capabilities: the character-card sidebar creates or reuses the latest empty root session, writes the character/playthrough directory, empty timeline, and catalog metadata, then rereads and validates. Renaming a generated `Playthrough N` edits the catalog and switches that row to a verbatim custom title. Empty-session greeting and imported-record preview hang on session-scoped `conversation.input.dock` and do not register a second conversation view. Imported records bind to the existing empty root session's import-context file. The opening footer provides bind, rebind, unbind, and last-three-QA preview. Real messages, an open turn, or a claimed binding lock the Host API.

Current implementation boundaries:

- Greeting, imported QA, and timelines never forge DSH user/assistant history messages; DSH still saves system text used in actual assembly. The first real assembly reads public `claimEventSeqs` from the same profile snapshot, then persists a successful pending → `claimed` transition and injects escaped, `untrusted`, read-only context. A view/assembly without a claim neither injects nor consumes. The same claimed identity may replay before terminal; a new claim after terminal does not inject.
- History first fixes a cut through `inspect`, then pages until Host `hasMore: false`. An empty page, illegal oldest `seq`, or repeating/non-advancing cursor returns 502 `PLAY_HISTORY_CURSOR_STALLED`; no summarization or slicing occurs.
- Catalog/timeline GET validates inside the target guard and returns the exact UTF-8-byte SHA-256 `revision`. PUT compares explicit `expectedRevision`, validates, writes a temporary file, and renames inside that guard. Missing fields, malformed revisions, and conflicts use stable 400/409 errors; a conflict leaves the file unchanged.
...
DSH `0.1.7-rc.1` separates three owners: Session owns lifecycle, Chat owns live messages, and Conversation owns views and interaction phase. The active main-view Session is selected from `useSessions` entries with `retainedBy.mainView > 0`; navigation uses public `uiWorkspace.openSession()`, not the removed `sessions.current/open` contract. Mirrors still supply lifecycle and workspace membership.
...
RP cached snapshots are bounded per client and keyed by playthrough path plus Session ID. Swipe transitions may explicitly retain the source Session's snapshot until the target is ready; unrelated retained Sessions cannot share a cache merely because they belong to one playthrough. Browser-only `conversationPhase` remains injected by `entry.js`. Regression covers first-send default RP, blank opening, partial increments, Trace, simultaneous retained owners, scope changes, and mode-exit disposal.
...
Runtime-load acceptance cares about: current selection, session isolation, append/replace, resource composition order, call config, final LLM messages/config/tools, official-history references, API audit, and the install entry. Changing load strategy later must not break format-parse tests with it.
