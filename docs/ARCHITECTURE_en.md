...
| DSH outer New session | Owned by the DSH `0.1.0-rc.8` sidebar shell; no public slot/service | Tavern does not take it over with hashed classes, DOM capture, or source replacement. Mowan keeps the native button and documents it as not recommended. Ordinary-area `+` only guides back to native |
