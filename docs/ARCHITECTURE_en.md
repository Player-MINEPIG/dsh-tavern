...
Clean sessions and configuration templates are stored by `packages/session-template` as a pure selection projection. The loader injects the real resource libraries and `SessionSelectionStore`. In DSH mode, the browser composition root creates/navigates an ordinary blank session only through public `uiWorkspace.connectWorkspace()` and `sessions.open()`. In Mowan, the same control plane first previews the configuration and obtains the character id, then reuses the shared playthrough controller and existing v2 atomic operations to create or reuse that character's playthrough, and finally writes the complete selection atomically through v1 apply. There is no dedicated “configure playthrough” backend verb. Neither path forks or forges history. Full transaction boundary: `LOADER_CONTRACT_en.md`.

## Loader-owned ActivationContext

DSH `0.1.5-rc.1` `agent/inbox/spliced` is a public, durable Session event. Insert, replace, cancel, and claim first append that event and synchronously notify `session/event`; only then does the live Inbox change. At this boundary the loader maintains the only `PendingInputProjection`; it uses `Session.ownEvents()` to exclude a fork's inherited prefix, then combines durable history with this step's claimed batch (after de-duplication) into a temporary `ActivationContext` for the world-book matcher to consume read-only. Other current-log reads are centralized on `session.seq` and `snapshotEvents()` and no longer access the removed `Session.events`.
...
| Clean new session / templates | DSH mode reuses `uiWorkspace.connectWorkspace()`; Mowan reuses the playthrough v2 session-controller composition; both navigate with `sessions.open()` | Tavern only copies selection atomically onto the target session. Mowan additionally treats the configured character as playthrough ownership and read-back-validates. It does not construct messages or fork history |
...
DSH `0.1.5-rc.1` separates three owners: Session owns lifecycle, Chat owns live messages, and Conversation owns views and interaction phase. The default RP adapter obtains the Conversation store handle from the public `conversation.session` entry and shares its instance by store handle × session scope. It neither takes `actions.setView()` from native `chat` nor creates another store. The invisible `conversation.input.dock` adapter handles only an unset `view`, then unregisters; explicit native Chat or Trace selections stay intact. A missing public handle leaves the enhancement uninstalled. Browser-only `conversationPhase` is injected into the composition root by `entry.js`, keeping ordinary modules importable in Node unit tests. Upgrade regression must cover first-send default RP, blank opening, partial increments, Trace, a single native `chat`, and mode-exit disposal.
