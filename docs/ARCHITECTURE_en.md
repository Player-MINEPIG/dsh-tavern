...
- A dedicated details block parser preserves inner Markdown without adding Markdown breaks to raw HTML layouts. `rich-text-styles.js` wraps purified content containing style in a per-message Shadow DOM with outer layout/paint containment. A React ref mounts it after insertion/updates; static exports use the same declarative roots. Template scripts remain blocked. Browser verification: `node scripts/verify-rich-text-browser.mjs` (Chrome/Chromium; optionally set `CHROME_PATH`).
