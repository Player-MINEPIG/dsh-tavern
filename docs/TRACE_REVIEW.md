...
以下为当前代码的验证结果；文档整理不改变运行时代码。

- `npm run check`：602 项、600 通过、0 失败、2 个条件跳过。已启用真实 AgentLoop 和
...
复验时将两个环境变量指向可解析目标 DSH 模块的 CLI 依赖目录（包含 `package.json`）：
...
路径见 `test/trace-v3-host.test.mjs`；合成模型和夹具不证明真实模型、真实卡片或对方插件已经验收。
