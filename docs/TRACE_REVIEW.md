...
- `npm run check`：645 项、643 通过、0 失败、2 个条件跳过。已启用真实 AgentLoop 和
...
- DT「诊断」集中展示当前 RP 工作区读取问题；Chrome 已验证三个真实旧周目的摘要关闭、
  单项/全部详情、技术详情折叠、重新检查、刷新页面后保留关闭偏好，以及原生/魔丸切换。
  窄窗口无横向溢出，合成页面的英文显示、工作区整体失败、恢复和复发提示通过；复制全部报告
  已通过本机粘贴核对 JSON 结构。新增 20 项状态/异步补绑测试覆盖并发读取、关闭偏好、存储不可用和取消队列。
  面板没有持久日志或新 API；三个旧周目的日志仍需从原环境或备份恢复。
