...
- ✅ catalog/timeline 已在 GET 读后与 PUT 写前执行同一 schema/path 校验；未知第三方 `ext` 原样保留。revision/CAS 仍待实现；路径锁、逐段 no-follow 检查、临时 `wx` 写和 rename 前复验已实现。
