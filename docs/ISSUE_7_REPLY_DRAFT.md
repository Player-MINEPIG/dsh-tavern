...
> 待维护者审核，尚未发送。用于 [#7](https://github.com/Player-MINEPIG/dsh-tavern/issues/7) 及其 A/B 实测补充。旧组合器与 #3 的复现结果见 [#3 回复草稿](ISSUE_3_REPLY_DRAFT.md)，本稿回答 v2.3.0 的 Trace API 与发布边界。

---

抱歉，之前没能及时回复你提出的 issue，让你久等了。

这里单独回复 **v2.3.0 提示词装配追踪与溯源**的正文读取问题，以及从 #3 移来的发布计划、公开接口边界。
...
当前 Tavern 段名为 `pmp-dsh-tavern:part:<ordinal>:<kind>:<field>`。前缀可以帮助官方接口消费者识别v2.3.0 中的 Tavern 段落，完整名字则便于展示与诊断；**目前不承诺完整拼接格式是跨版本稳定的字段提取接口**，但会尽量保持稳定。
...
历史查看器应依赖 v3 文档定义的字段、来源关系、引用和不可用状态，不需要解析段名来还原这些语义；段内多个来源仍只保证贡献关系，不保证逐字归属。运行期若按当前段名匹配，应固定版本并报告零匹配或歧义，不能静默贡献空段或复用上一轮缓存。v3 的历史来源关系也不能充当本轮装配前的实时输入。
...
已有 user/assistant 历史保持原有时序，不通过搬动历史消息来实现强调。来源 metadata 可以供消费者分类和制定自己的重要性策略，但本版不提供模型消息权重接口，也不把来源标识自动塞进模型正文。本版只近似放置 PHI，不能宣称已完整实现 ST 的位置语义。
...
已决定将 [`codex/trace-api-v3`](https://github.com/Player-MINEPIG/dsh-tavern/tree/codex/trace-api-v3) 合并到 `main`，作为 **v2.3.0**，目标 DSH `0.1.5-rc.1`。本版主题是提示词装配追踪与溯源：Tavern Trace 与第三方共用只读 v3 接口，查看已捕获请求的段落、正文和来源关系。技术检查和呈现审核已完成，目前等待文档、发布公告与回复内容审核，审核后再执行合并；尚未合并、打 tag 或发布。

2.2.0 没有这些 v3 入口，因此不能把 2.2.0 用户直接导向它们。审核期间联调请固定上述分支的提交，发布后再依据对应 tag 的文档接入。现有验证包含 654 项检查（652 通过、0 失败、2 条件跳过）、真实 Host 冷读与 Chrome 验收；你们实际插件按新合同的联调和原生 Windows 验证仍未完成，详见 [验收范围](https://github.com/Player-MINEPIG/dsh-tavern/blob/codex/trace-api-v3/docs/TRACE_REVIEW.md)。
...
v3 的主要公开合同是文档定义的只读路由与结构化响应语义，包括段落、来源关系和官方历史引用，不是对完整段名字符串的解析约定。v2.3.0 的 v3 只有 capabilities、装配索引、单条详情三个只读入口。`apiVersion: 3` 和 `contract: "prompt-trace-primitives"` 用于识别 API 类别，**不是 Tavern 版本号，也不能证明旧 `/sources` 仍存在**；你们这次两版 contract 一样但端点不同，正说明仅检查这两项不够。发布前联调需固定提交，发布后依照版本文档接入，并按实际接口返回处理不兼容，不能静默降为空内容。

已将上述边界和迁移指路补入 [v3 文档](https://github.com/Player-MINEPIG/dsh-tavern/blob/codex/trace-api-v3/docs/PROMPT_API_V3.md)。关于 410 或弃用期的建议，v2.3.0 未提供过渡别名或 410，`/sources` 仍返回 404；这份回复不将建议写成已完成能力。正式版本的合同以对应 tag 文档为准。
