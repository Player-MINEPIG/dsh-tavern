...
| 刷新恢复 | record 从 `tavern-traces.json` 重建并由 GET API 返回 |
| 数据上限 | 测试配置 2 sessions × 2 records，旧 bucket 和旧 record 被稳定裁剪 |
...
本分支最终自动结果：`npm run check` 为 85 passed、0 failed、1 optional
external-fixture test skipped；`npm run pack:check` 成功并预览 40 个发布文件。
