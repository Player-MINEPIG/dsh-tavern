...
- `agent/inbox/spliced`：公开 Session event，插入、替换、取消与 claim 均先 append，并同步通知 `session/event` 观察者；loader 投影 claimed input，Trace 不直接拥有该投影；
...
保存的 record 包含 `turn/step/attempt`、资源 id/name 摘要、profile 长度与 hash、支持的 Tavern call-config 字段、诊断、world-book budget 和 entry decisions。Entry decision 分开保存条目配置的主/附加关键词与本轮实际命中的关键词，并包含 secondary logic、概率阈值与显式 roll、token cost、接受/拒绝原因、请求位置与 loader 实际位置。preset、角色卡和用户保持资源级激活摘要；当前简洁模式不为其中某一个资源单独扩张插入轨迹。
...
正文没有“显示完整内容”开关。用户需要确认最终 system/config 时应查看 DSH request/header；Trace 只提供激活资源摘要和 matcher 决策解释。若未来加入详细模式，应一次覆盖 preset prompt、角色字段、用户描述和 lore 的完整装配顺序，而不是只追踪其中一个资源。
...
- world-book 按 system assembly 当时的 `ActivationContext`（durable history + 去重后的 claimed input）匹配；Trace 复用 exact assembly snapshot，不在稍后的 hook 重新计算。记录只新增消息数、字符数、截断状态和 claim event seq，不保存输入正文或正文哈希。
