...
显示正则页按全局、当前预设、当前角色卡三个来源直接陈列规则。拖动规则标题左侧的横杠可调整同一来源内的执行顺序；点击“保存修改”后，全局顺序写入工作区正则文档，预设/角色卡顺序写回各自原生 `regex_scripts` 数组。不同来源不能互相拖动，最终组合顺序固定为全局 → 预设 → 角色卡。正则从上到下执行，因此条件清空、标签提取等相互依赖的规则必须按预期排列。
...
魔丸完全隐藏 reasoning、子 agent 报告、完成通知、工具 context 等运行细节，不提供展开按钮；需要查看时切回 DSH 原生“对话”视图。由这些上下文触发的父 agent 输出若有 RP 正文，仍可复制、分支、做显示层编辑或隐藏，但不能“重新生成/滑动”，因为那会错误地把上下文报告重新发送成用户消息。由真实用户消息触发的普通回复不受影响。若显示正则把某段 assistant 输出完全清空，该输出和按钮一起不显示，非可视来源数据与周目指针仍保留。
