...
DSH 注入的子 agent 报告、完成通知等运行时上下文在魔丸中显示为默认折叠的“上下文注入”，不会伪装成用户气泡。展开只用于查看原始注入内容；折叠行没有复制、滑动、编辑等 Tavern 消息按钮。由这些上下文触发的父 agent 输出仍可复制、分支、做显示层编辑或隐藏，但不能“重新生成/滑动”，因为那会错误地把上下文报告重新发送成用户消息。由真实用户消息触发的普通回复不受影响。若显示正则把某段 assistant 输出完全清空，该输出和按钮一起不显示，折叠上下文与周目指针仍保留。
