...
点击回复下方的“修改显示文字”后，编辑器直接在该回复位置展开为可拉伸的多行输入框，不调用浏览器单行 prompt。保存只更新 timeline 的 `displayOverride`，取消或按 Esc 放弃本次修改；DSH 原始 assistant 消息和后续模型上下文都不会改变。“恢复原回复”清除该显示覆盖。
