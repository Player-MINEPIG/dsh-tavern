...
| 功能按钮与周目分支 | 已实现，待统一验收 | hidden/displayOverride、已有 variant 切换、回复 swipe、中断 durable 前缀交接，以及从 adopted 回复创建可继续对话的新周目。显示编辑已改为 conversation 内多行编辑器；覆盖冻结为最终显示文本，空覆盖仍可恢复。 |
| 显示正则顺序 | 已实现，待统一验收 | 全局、预设、角色卡各自支持拖拽排序；保存分别写工作区文档或原生 `regex_scripts` 数组。跨来源禁止拖动，组合顺序固定全局→预设→角色卡。 |
