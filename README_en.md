...
> This page describes `2.3.2`, which fixes inherited pending input in rollback, branches, and non-first-turn swipes. The runtime target remains DSH `0.1.5-rc.1`; compatibility with newer DSH releases is unverified. See the [changelog](CHANGELOG.md). [MIT License](LICENSE).
>
...
Install `2.3.2` from GitHub using its version tag:
...
```sh
dsh plugin --profile web add github:Player-MINEPIG/dsh-tavern#v2.3.2
```
...
```sh
git clone --branch v2.3.2 https://github.com/Player-MINEPIG/dsh-tavern.git
cd dsh-tavern
